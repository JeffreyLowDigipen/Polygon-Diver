...
object EnemySpawner {
    ...
    public val enemiesPerSpawn = 6 // Number of enemies to spawn at once
    public val spawnDistanceMultiplier = 0.1f // Distance factor outside of the screen
    ...
    private fun spawnEnemies(count: Int) {
        ...
        for (i in 1..count) {
            ...
            //Log.d("Enemy","Enemy Spawned at ${spawnPosition.x },${spawnPosition.y}")
        }
    }

    private fun generateSpawnPositionOutsideCamera(): Vector3f {
        val cameraPos = GameObjectManager.camera.cameraPosition
        val cameraScale = GameObjectManager.camera.CheckBackgroundScale() // Assuming screen resolution
